...
export function materializeOutcome(args: {
  ...
  const pricing = contract.pricing as {
    USDC?: { type?: unknown; fixed_amount?: unknown };
  } | undefined;
  const fixedAmount = pricing?.USDC?.fixed_amount;
  if (
    pricing?.USDC?.type !== "one-time" ||
    typeof fixedAmount !== "string" ||
    !/^[1-9]\d*$/.test(fixedAmount)
  ) {
