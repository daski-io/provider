...
interface EvidenceAdmissionDatabase {
...
export async function admitStandardEvidence(
  ...
  database: EvidenceAdmissionDatabase = pool,
): Promise<void> {
      ...
      const admitted = await client.query(
        `INSERT INTO standard_evidence_admissions
          (evidence_hash,order_id,evidence_kind,transaction_hash,block_number,block_hash,
           authorization_key,release_sequence,
           transaction_index,log_index,source_fingerprints,canonical_evidence)
        ...
        [
          ...
          item.blockNumber, item.blockHash, kind === "deposit" ? bytes(authorizationKey) : null,
          kind === "release" ? bundle.release.releaseSequence : null,
          item.transactionIndex, item.logIndex,
          JSON.stringify(item.sources), item.canonicalEvidence,
        ],
      );
