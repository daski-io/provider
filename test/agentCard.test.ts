...
// Stub the registry so "dummy" resolves to a sample item lifecycle.
              ...
              assetLifecycle: {
                item: {
                  ...
                  transitions: [
                    ...
                    { from: "active", to: "active", skill: "update-item" },
                    {
                      from: "active",
                      to: "deleted",
                      skill: "archive-item",
...
describe("agent card generator", () => {
  ...
  it("emits service taxonomy and per-skill fulfillment without the legacy category", () => {
    ...
    expect(ext.serviceType).toBe("dummy");
    ...
    expect(card.skills[0]?.tags).toEqual(
      expect.arrayContaining(["other", "dummy"]),
    );
  });
  ...
  it("emits per-skill pricing under extensions[uri].skills[skillId].pricing", () => {
    ...
    expect(skills["create-item"].serviceSlug).toBe("dummy");
  ...
  it("surfaces the humanParties marker per skill when declared (migration 009 / S13-S14)", () => {
    const card = generateAgentCard(makeService(), [
      makeSkill({ skill_id: "create-item", human_parties: "required" }),
      makeSkill({ skill_id: "update-item", human_parties: "none" }),
      makeSkill({ skill_id: "file-compliance", human_parties: "varies" }),
    ]);
    ...
    expect(skills["update-item"].humanParties).toBe("none");
    expect(skills["file-compliance"].humanParties).toBe("varies");
  });
  ...
  it("emits an A2A v1.0-shaped supportedInterfaces entry (no top-level url/protocolVersion)", () => {
    ...
    expect(iface.url).toBe("https://provider.test/a2a/dummy");
  ...
  it("emits assetTypes lifecycle map for asset-lifecycle services that touch known asset types", () => {
    ...
    // Canonical AssetStatus vocabulary (matches what get-item reports).
