...
describe("standard asset hardening migration against PostgreSQL", () => {
  it("admits provider-action rate buckets and forbids stored completed results", async () => {
    ...
    try {
      ...
      await client.query(`CREATE TABLE standard_asset_rate_buckets (
        ...
        CONSTRAINT standard_asset_rate_buckets_scope_check
          CHECK (scope IN ('gateway-signer','payer','global'))
      )`);
      await client.query(`CREATE TABLE standard_asset_action_executions (
        ...
        CONSTRAINT standard_asset_action_result_check CHECK (
          (state='completed' AND error_class IS NULL AND
            (sanitized_result IS NOT NULL OR replay_policy IN
              ('regenerate-ephemeral','redacted-after-window') OR result_redacted_at IS NOT NULL))
      ...
      await client.query(
        `INSERT INTO standard_asset_action_executions
         ...
         VALUES ('old-result','completed','stable-result','{"legacy":true}')`,
      );
      ...
      const migration = readFileSync(
        new URL("../src/core/db/migrations/044_standard_wallet_reputation_hardening.sql", import.meta.url),
        "utf8",
      );
      await client.query(migration);
      await expect(consumeAssetEndpointRate({
        db: client,
        gatewaySigner: "0x3333333333333333333333333333333333333333",
      ...
      const legacy = await client.query<{ sanitized_result: unknown }>(
        "SELECT sanitized_result FROM standard_asset_action_executions WHERE id='old-result'",
      );
      expect(legacy.rows[0]?.sanitized_result).toBeNull();
